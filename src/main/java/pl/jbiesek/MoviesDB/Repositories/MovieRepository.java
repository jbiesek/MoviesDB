...
public interface MovieRepository extends JpaRepository<Movie, Integer> {
    ...
    @Query(value = "select m.id, m.title, m.country, m.year, m.description, m.duration, m.director_id from movie m join director d on m.director_id = d.id where d.id = 1;", nativeQuery = true)
